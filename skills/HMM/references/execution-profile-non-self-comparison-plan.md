...
本文中的 `autonomous_deep` 与 `autonomous_steady` 是内部 comparison label。
实际执行 prompt 时，对外关键词应分别使用 `自治深探` 与 `自治稳推`。
...
8. 是否在合理时机停止
9. 实际产物集合是否覆盖本轮 required outputs
10. 是否存在 missing stage outputs
